...
from val_strict_isp import val_strict_isp, window_bounds
...
def _monte_carlo_acceptance_probability(case: YCSigBenchmarkCase, scheme: YCSig) -> float:
    ...
    for _ in range(case.acceptance_samples):
        ...
        if val_strict_isp(partition_value, scheme.params.pm_ISP) is not None:
            accepted += 1
        ...
        lines.append(
            (
                ...
                + (
                    ...
                    if analytic["signature_hash_equivalents_override"] is not None
                    else ""
                )
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="exact",
        help="How to estimate the ValStrictISP acceptance probability.",
    )
    parser.add_argument(
        "--acceptance-samples",
        type=int,
