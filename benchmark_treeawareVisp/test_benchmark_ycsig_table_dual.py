...
from benchmark_ycsig_table_dual import _format_latex, _format_text, run_paper_table_dual
...
class BenchmarkYCSigTableDualTests(unittest.TestCase):
    ...
    def test_format_latex_includes_table_environment(self) -> None:
        results = run_paper_table_dual(
            samples=1,
            repetitions=1,
            cpu_frequency_ghz=1.0,
        ...
        rendered = _format_latex(results)

        self.assertIn(r"\begin{table}[t]", rendered)
        self.assertIn(r"\label{tab:ycsig-dual-ops}", rendered)
        self.assertIn(r"\label{tab:ycsig-dual-cycles}", rendered)
